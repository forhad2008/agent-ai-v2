...
export const MobileNavBar: React.FC<MobileNavBarProps> = ({ onOpenMobileMenu }) => {
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-40 flex h-16 items-center justify-around border-t border-[#10B981]/25 bg-[#02140d]/60 px-2 backdrop-blur-[32px] lg:hidden safe-area-pb shadow-2xl"
    >
      {/* 1. Home */}
      <button
        id="mobile_nav_home"
        ...
        className={`relative flex flex-1 flex-col items-center justify-center py-1 transition-all ${
          activeView === 'dashboard' ? 'text-[#00D9A5] font-bold' : 'text-[#94A3B8] hover:text-[#F8FAFC]'
        }`}
        aria-label="Home"
      >
        ...
        {activeView === 'dashboard' && (
          <span className="absolute bottom-0.5 h-1 w-6 rounded-full bg-[#10B981] shadow-[0_0_8px_#10B981]" />
        )}
      </button>
      ...
      <button
        ...
        className={`relative flex flex-1 flex-col items-center justify-center py-1 transition-all ${
          activeView === 'chat' ? 'text-[#00D9A5] font-bold' : 'text-[#94A3B8] hover:text-[#F8FAFC]'
        }`}
        aria-label="AI Chat"
      >
        <div className="relative">
          ...
          {isGenerating && (
            <span className="absolute -top-1 -right-1 flex h-2.5 w-2.5">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#00D9A5] opacity-75" />
      ...
      <button
        ...
        className={`relative flex flex-1 flex-col items-center justify-center py-1 transition-all ${
          activeView === 'tasks' ? 'text-[#00D9A5] font-bold' : 'text-[#94A3B8] hover:text-[#F8FAFC]'
        }`}
        aria-label="Tasks"
      >
        <div className="relative">
          ...
          {activeTasksCount > 0 && (
            <span className="absolute -top-1.5 -right-2.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-[#14532d] px-1 text-[9px] font-mono font-bold text-[#00D9A5] border border-[#10B981]/40">
              {activeTasksCount}
            </span>
          )}
        </div>
      ...
      <button
        ...
        className={`relative flex flex-1 flex-col items-center justify-center py-1 transition-all ${
          activeView === 'approvals' ? 'text-[#00D9A5] font-bold' : 'text-[#94A3B8] hover:text-[#F8FAFC]'
        }`}

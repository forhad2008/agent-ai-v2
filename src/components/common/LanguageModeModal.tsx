  ...
  return (
    ...
    >
      <div
        ...
        className="relative flex flex-col w-full max-w-2xl max-h-[90vh] rounded-3xl bg-[#010e09] border border-[#10B981]/30 overflow-hidden text-[#F8FAFC] shadow-[0_0_50px_rgba(16,185,129,0.3)] animate-scaleUp"
      >
        {/* Modal Header */}
        <div className="flex items-start justify-between border-b border-[#10B981]/20 p-4 sm:p-6 bg-gradient-to-r from-[#010e09] via-[#021c13] to-[#010e09]">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#10B981]/20 border border-[#10B981]/30 text-[#00D9A5]">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-[#10B981]/20 px-2 py-0.5 text-[10px] font-mono text-[#00D9A5] border border-[#10B981]/40">
                  30 Tech Nations
                </span>
              </div>
          ...
          <button
            ...
            onClick={() => setIsLanguageModalOpen(false)}
            className="rounded-xl p-2 text-[#94A3B8] hover:bg-[#02140d] hover:text-[#F8FAFC] transition-colors cursor-pointer"
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-2.5 bg-[#010e09] border-b border-[#10B981]/20 text-xs">
          <div className="flex items-center gap-2">
            <span className="text-[#94A3B8]">Active Mode:</span>
            <span className="text-base">{currentLanguage.flag}</span>
            <span className="font-semibold text-[#00D9A5]">{currentLanguage.country}</span>
            <span className="text-[#94A3B8]">({currentLanguage.name})</span>
          </div>
        </div>

        {/* Search & Region Filter Bar */}
        <div className="p-4 sm:px-6 sm:py-3 border-b border-[#10B981]/20 space-y-3 bg-[#010e09]">
          {/* Search Box */}
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl bg-black/40 pl-9 pr-4 py-2 text-xs sm:text-sm text-[#F8FAFC] placeholder:text-[#94A3B8]/60 border border-[#10B981]/30 focus:outline-none focus:border-[#00D9A5]"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
            ...
            {regions.map((reg) => (
              <button
                ...
                className={`whitespace-nowrap px-3 py-1.5 rounded-lg font-medium transition-all cursor-pointer ${
                  regionFilter === reg
                    ? 'bg-[#10B981]/30 text-[#00D9A5] border border-[#10B981]/60 shadow-sm'
                    : 'bg-[#02140d] text-[#94A3B8] hover:bg-[#065f46]/30 hover:text-[#F8FAFC] border border-transparent'
                }`}
              >
                {reg}
              </button>
                ...
                return (
                  <button
                    ...
                    className={`text-left flex items-center justify-between p-3 rounded-xl transition-all duration-200 group border cursor-pointer ${
                      isActive
                        ? 'bg-gradient-to-r from-[#10B981]/20 to-[#854D0E]/20 border-[#10B981]/40 text-[#F8FAFC] shadow-[0_0_15px_rgba(16,185,129,0.12)]'
                        : 'bg-[#02140d]/40 hover:bg-[#065f46]/20 border-white/5 hover:border-[#10B981]/30 text-[#94A3B8] hover:text-[#F8FAFC]'
                    }`}
                  >
                    ...
                    <div className="flex items-center shrink-0 ml-2">
                      {isActive ? (
                        ...
                        <ChevronRight className="h-3.5 w-3.5 text-slate-500 group-hover:text-[#00D9A5] group-hover:translate-x-0.5 transition-all duration-200" />
                      )}
                    </div>
                  </button>
                );
        ...
        {/* Modal Footer */}
        <div className="flex items-center justify-between p-4 sm:px-6 bg-[#010e09] border-t border-[#10B981]/20 text-xs">
          <span className="text-[#94A3B8]">
            Selected: <strong className="text-[#F8FAFC]">{currentLanguage.country}</strong> ({currentLanguage.englishName})
          </span>
          <button
            onClick={() => setIsLanguageModalOpen(false)}
            className="rounded-xl bg-[#02140d] hover:bg-[#065f46]/30 px-4 py-2 font-semibold text-[#00D9A5] border border-[#10B981]/30 transition-colors cursor-pointer"
          >
            {t.closeModal}
          </button>
        </div>

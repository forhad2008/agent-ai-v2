...
export const ToolExecutionCard: React.FC<ToolExecutionCardProps> = ({ tool }) => {
  ...
  const getToolIcon = () => {
    switch (tool.category) {
      case 'WEB_TOOLS':
        return <Globe className="h-3.5 w-3.5 text-[#00D9A5]" />;
      case 'CODE_TOOLS':
        return <Code2 className="h-3.5 w-3.5 text-[#854D0E]" />;
      case 'FILE_TOOLS':
  ...
  return (
    <div className="my-2 rounded-xl bg-[#010e09] border border-[#10B981]/25 text-xs overflow-hidden">
      <div
        onClick={() => setExpanded(!expanded)}
        className="flex cursor-pointer items-center justify-between px-3.5 py-2.5 hover:bg-[#10B981]/15 transition-colors"
      >
        <div className="flex items-center gap-2">
          {getToolIcon()}
          <span className="font-mono text-[#F8FAFC] font-medium">{tool.toolName}</span>
          <span className="rounded-lg bg-[#010e09] px-2 py-0.5 text-[10px] text-[#94A3B8] border border-[#10B981]/20">
            {tool.category}
          </span>
        </div>
      ...
      {expanded && (
        <div className="border-t border-[#10B981]/20 bg-[#010e09] px-3.5 py-2.5 text-[#F8FAFC]">

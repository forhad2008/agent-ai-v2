  ...
  return (
    <div
      ...
      className={`my-4 overflow-hidden rounded-2xl border backdrop-blur-md transition-all shadow-xl ${
        isApproved
          ? 'bg-[#02140d]/90 border-[#00D9A5]/40 text-[#F8FAFC]'
          : isRejected
          ? 'bg-[#02140d]/90 border-rose-500/40 text-[#F8FAFC]'
          : 'bg-[#02140d]/90 border-[#10B981]/25 shadow-[0_0_25px_rgba(16,185,129,0.15)]'
      }`}
    >
      {/* Header Banner */}
      <div
      ...
      >
        <div className="flex items-center gap-2">
          {isApproved ? (
            ...
            <ShieldAlert className="h-4 w-4 text-[#00D9A5] animate-pulse" />
          )}
        ...
        <div className="flex items-center gap-1.5 rounded-lg bg-[#010e09] px-2.5 py-1 text-[10px] font-mono border border-[#10B981]/30">
          <AlertTriangle className="h-3 w-3 text-[#00D9A5]" />
          <span className="text-[#00D9A5] font-semibold">{approval.riskLevel}</span>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5 space-y-3.5 text-xs">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[#94A3B8]">
          <div className="rounded-xl bg-[#010e09] p-2.5 border border-[#10B981]/15">
          ...
          <div className="flex items-center justify-between mb-1.5">
            ...
            {isPending && !isEditing && (
              <button
                ...
                className="flex items-center gap-1 text-[11px] text-[#00D9A5] hover:text-[#10B981]"
              >
          ...
          {isEditing ? (
            <div className="space-y-2">
              <textarea
                ...
                rows={4}
                className="w-full rounded-xl bg-[#010e09] p-3 text-xs text-[#F8FAFC] border border-[#10B981]/60 focus:border-[#00D9A5] focus:outline-none"
              />
              <div className="flex justify-end gap-2">
                <button
                  onClick={() => setIsEditing(false)}
                  className="rounded-lg px-2.5 py-1 text-[11px] text-[#94A3B8] hover:bg-[#010e09]"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSaveEdit}
                  className="rounded-lg bg-[#10B981]/20 px-3 py-1 text-[11px] font-medium text-[#00D9A5] border border-[#10B981]/40 hover:bg-[#10B981]/30"
                >
                  Save Changes
                </button>
              </div>
            ...
            <div className="rounded-xl bg-[#010e09] p-3 text-[#F8FAFC] border border-[#10B981]/20 font-mono text-[11px] leading-relaxed whitespace-pre-wrap">
              {editedPreview}
            </div>
          )}
        </div>
        ...
        {/* Actions Button Bar */}
        {isPending ? (
          <div className="pt-2 flex flex-wrap items-center justify-end gap-2 border-t border-[#10B981]/20">
            <button
              id={`btn_regenerate_${approval.id}`}
              onClick={handleRegenerate}
              className="flex items-center gap-1.5 rounded-xl bg-[#010e09] px-3 py-2 text-xs font-medium text-[#94A3B8] border border-[#10B981]/25 hover:bg-[#10B981]/15 hover:text-[#F8FAFC] transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-[#065f46] via-[#854D0E] to-[#10B981] px-4 py-2 text-xs font-bold text-white shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:brightness-110 transition-all"
            >
          ...
          <div className="pt-2 flex items-center justify-between border-t border-[#10B981]/20 text-[11px] text-[#94A3B8]">

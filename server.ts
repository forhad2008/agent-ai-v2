...
function getSystemInstruction(language: string = "en", userProfile?: any): string {
  ...
  if (isBangla) {
    return `You are Agent-forest08, ${userName}'s personal AI Agent. You must introduce yourself as Agent-forest08 in everywhere and act & work as Agent-forest08. You are assisting ${userName}${userRole}${company}.${customInstructions}${techStack}${goals}
  ...
  try {
    ...
    if (isIdentityQuery) {
      return res.json({
        ...
        thinking: "Identity query intercepted. Responding with official agent name: I am Agent-forest08.",
        planSteps: [
          { title: "Analyze identity request", status: "completed" },
          { title: "Recall official profile", status: "completed" }
        ],
        toolExecutions: [
          { toolName: "identity_resolver", category: "SYSTEM_TOOLS", status: "success", description: "Resolved Agent-forest08 official identity details" }
        ],
